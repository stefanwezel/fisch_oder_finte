...
if __name__ == '__main__':
    ...
    if not os.path.exists(args.fishes_csv):
        warnings.warn(f"The specified csv {args.fishes_csv} does not exists... Please make sure to enter a valid filename...")

    real_fishes = pd.read_csv(args.fishes_csv).name.to_list()
    made_up_fishes = pd.read_csv("made_up_fishes.csv").name.to_list()
    points = 0
    ...
    try:
        while True:
            ...
            print("Druecke 'Enter' um weiterzuspielen oder eine beliebige andere Taste um das Spiel zu beenden.")
            key_pressed = getkey()
            if key_pressed != key.ENTER:
                print("-"*50)
                print(f"Das Spiel wurde beendet. Du hast insgesamt {points} erreicht.")
                break
            print("-"*50)
